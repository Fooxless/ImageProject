...
import { useImageRedis } from "./APIs/useImageRedis.js";
...
function HistoryImage() {
  const { ImageResize2 } = useParams();
  // eslint-disable-next-line no-unused-vars
  const { redisLoading, imageRedis, imagerror } = useImageRedis(ImageResize2);
  ...
  function renderRow(props) {
    ...
    return (
      <ListItem style={style} key={index} component="div" disablePadding>
        <ListItemButton>
          {
            history?.length !== 0 && history.keyz[index] !== null ? <ListItemText primary={`${history.keyz[index]}`}
              onClick={(button) => {
                navigate(`/history/${history.keyz[index]}`)
                handleClose();
              }} /> : null
          }
        </ListItemButton>
  ...
  if (!redisLoading) {
    ImageURL = `https://cab432-assignment2-cg-ms.s3.ap-southeast-2.amazonaws.com/${imageRedis?.data[0].name}`
  }
    ...
    <div className='Page'>
      <div className='body'>
        ...
        <div style={{ height: 400, width: '50%', left: "10%", position: "relative" }}>
          <DataGrid
            rows={redisLoading ? ([]) : (imageRedis?.data)}
        ...
        <Button variant="contained" sx={{ mt: 5, ml: 5 }} onClick={() => navigate(`/`)}>
          New
        </Button>

        {redisLoading ? (<Box sx={{ mt: 10 }}>
